...
int main(int argc, char* argv[])
{
    try {
        ...
        while (std::getline(std::cin, line)) {
            requestCount++;
            MCP_LOG_DEBUG("Received request #" + std::to_string(requestCount) + ": " + line);
            ...
            if (!line.empty()) {
                ...
                if (!response.empty()) {
                    MCP_LOG_DEBUG("Sending response #" + std::to_string(requestCount) + ": " + response);
                    std::cout << response << std::endl;
                    std::cout.flush();
                } else {
                    MCP_LOG_WARN("Empty response generated for request #" + std::to_string(requestCount));
                }
            } else {
